...
function detectStructure(lower: string): string {
  if (/eiffel/.test(lower)) return "eiffel";
  ...
  if (/lattice/.test(lower)) return "lattice";
...
export function heuristicPromptToDesign(
  ...
  // ── StructureGraph path (true-scale lattice / Eiffel) ──────────────
  if (kind === "eiffel" || kind === "lattice") {
    const graph = buildLatticeTowerGraph({
    ...
    result = {
      ...
      notes: [
        ...
        mapped.spliceCount > 0
          ? `${mapped.spliceCount} splice joint(s) where members exceed stock length`
          : "No splices — each member fits in one stock piece",
        `Joins: ${mapped.joinSummary.join(", ") || "glue"}`,
        "Build base ring first, then legs bay-by-bay, then braces, tip last.",
      ],
    };
  } else {
    switch (kind) {
  ...
  // Cap instance count so canvas stays snappy (higher for lattice)
  const MAX = kind === "eiffel" || kind === "lattice" ? 480 : 220;
  let instances = result.instances;
  if (instances.length > MAX) {
    instances = instances.slice(0, MAX);
    result.notes.push(
      `Limited to ${MAX} pieces for performance — raise bay density later or refine freehand.`
    );
  }
...
export const DREAM_EXAMPLES = [
  ...
  "lattice tower 24 inches from craft sticks",
